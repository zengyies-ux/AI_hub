...
class 新闻爬虫:
    ...
    def 获取新浪国际新闻(self) -> List[新闻文章]:
        ...
        try:
            # 新浪新闻API接口
            api_urls = [
                "https://feed.sina.com.cn/api/roll/get?pageid=153&lid=2512&num=20&versionNumber=1.2.4&page=1&encode=utf-8",
                "https://interface.sina.cn/news/get_news_by_channel.d.json?channel=world&page=1&num=20",
            ]
                ...
                try:
                    ...
                    for 新闻 in 新闻数据[:20]:
                        if isinstance(新闻, dict):
                            标题 = 新闻.get('title', '')
                            链接 = 新闻.get('url', '') or 新闻.get('link', '')
                            
                            if 标题 and 链接:
                                if len(标题) > 5 and len(标题) < 150:
        ...
        # 方法2: 使用Playwright爬取
        if self.browser and len(新闻列表) < 5:
            ...
            try:
                ...
                for 网址 in urls:
                    try:
                        html_content = self._使用Playwright爬取(网址, 等待时间=10)
                        ...
                        if html_content:
                                            ...
                                            if 文本 and len(文本) > 5 and len(文本) < 150:
                                                if not any(关键词 in 文本 for 关键词 in ['广告', '推广', 'APP', '下载', '新浪', '登录', '注册', '更多', '首页']):
                                                    if 'sina.com.cn' in 链接 or 链接.startswith('/'):
                                                        ...
                                                        # 检查是否已存在
                                                        if not any(文章.标题 == 文本 for 文章 in 新闻列表):
                            ...
                            if len(新闻列表) >= 8:
                                print(f"   成功从Playwright获取 {len(新闻列表)} 条新浪新闻")
                                return 新闻列表[:10]
        ...
        if len(新闻列表) < 5:
            ...
            try:
                ...
                响应 = self._带重试请求("https://news.sina.com.cn/world/")
                响应.encoding = 'utf-8'
                汤 = BeautifulSoup(响应.text, 'html.parser')
                ...
                print(f"   传统方法失败: {str(错误)[:50]}")
        
        print(f"   总共获取 {len(新闻列表)} 条新浪新闻")
        return 新闻列表[:10]
    ...
    def 获取腾讯国际新闻(self) -> List[新闻文章]:
        ...
        try:
            # 腾讯新闻API接口
            api_urls = [
                "https://i.news.qq.com/trpc.qqnews_web.kv_srv.kv_srv_http_proxy/list?sub_srv_id=world&srv_id=pc&limit=30&page=1",
            ...
            for api_url in api_urls:
                try:
                    ...
                    # 解析API响应
                    新闻数据 = []
                    if 'data' in 数据 and isinstance(数据['data'], dict):
                        if 'list' in 数据['data']:
                            新闻数据 = 数据['data']['list']
                    elif 'data' in 数据 and isinstance(数据['data'], list):
                        新闻数据 = 数据['data']
                    ...
                    if len(新闻列表) >= 5:
                        print(f"   成功从API获取 {len(新闻列表)} 条腾讯新闻")
                        return 新闻列表[:10]
                ...
                for 网址 in urls:
                    try:
                        ...
                        if html_content:
                            ...
                            if len(新闻列表) >= 8:
                                print(f"   成功从Playwright获取 {len(新闻列表)} 条腾讯新闻")
                                return 新闻列表[:10]
        ...
        if len(新闻列表) < 5:
            ...
            try:
                ...
                响应 = self._带重试请求("https://new.qq.com/ch/world/")
                响应.encoding = 'utf-8'
                汤 = BeautifulSoup(响应.text, 'html.parser')
